...
/*
 * 
 * cat code | hexdump -v -e '/1 "%02x"' | sed 's/\(..\)/%\1/g' > codeencoded edit codeencoded and
 * add 'code=' at the beginning wget -d -S -O - --post-file=./codeencoded
 * http://localhost:8990/dyn/slots/jsx/
 */

@Provides @Named("JSXTask")
public static ReadyTask runJSX(@Named("Configuration") Properties config) {

	String nodeFolder = config
			.getProperty(
					SPCellSlotParserComponent.NODEFOLDER_PROPERTY,
					SPCellSlotParserComponent.NODEFOLDER);
	String tsNodeCommand = config
			.getProperty(
					SPCellSlotParserComponent.TSNODE_PROPERTY,
					SPCellSlotParserComponent.TSNODE_PATH);
	String tsCode = config
			.getProperty(
					SPCellSlotParserComponent.TSCODE_PROPERTY,
					SPCellSlotParserComponent.TSCODE_PATH);
	var command = "PATH=$PATH:" + nodeFolder + " " + tsNodeCommand + " " + tsCode + " --jsx";

	return DaggerExecTaskComponent
			.builder()
...
public static String slots(	@Named("JSXTask") ReadyTask task,
							String code) {
	...
	try {
		...
		return (finished.isOK() && finished.result() == 0) ? start.show() + running.show()
				: OUTPUT_ERROR;
...
public static String codeSlots(	@Named("Slots") String slots,
								String code,
...
public static String content(	@Named("CodeSlots") String codeSlots,
								String code,
